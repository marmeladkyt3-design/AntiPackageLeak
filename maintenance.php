<?php
$maintenance_data = json_decode(file_get_contents(__DIR__ . '/maintenance.json'), true);
...
$current_year = date('Y');
$maint_message = htmlspecialchars($maintenance_data['message'] ?? 'Сервис временно недоступен. Мы уже работаем над устранением проблемы. Попробуйте вернуться позже.');
?>
<!DOCTYPE html>
<html lang="ru">
<head>
...
<title>503 — Техническое обслуживание | <?php echo $site_name; ?></title>
<style>
...
<main class="error-page">
    ...
    <p><?php echo $maint_message; ?></p>
    <div class="error-btns">
        <a href="/main" class="error-btn primary"><i class="fas fa-home"></i> На главную</a>
        <?php if (!isLoggedIn()): ?>
            <a href="/login" class="error-btn outline"><i class="fas fa-sign-in-alt"></i> Войти</a>
        <?php endif; ?>
    </div>
</main>
