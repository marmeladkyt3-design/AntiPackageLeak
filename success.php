...
$order_id = '';
if (isset($_GET['o'])) {
    $order_id = (string)substr(trim($_GET['o']), 0, 64);
} elseif (isset($_GET['order_id'])) {
    $order_id = (string)substr(trim($_GET['order_id']), 0, 64);
}
$paid_ok = false;
if ($order_id !== '' && preg_match('/^[a-z0-9]+_(\d+)_/', $order_id, $m)) {
    try {
        $st = $pdo->prepare("SELECT status FROM payments WHERE order_id = ? LIMIT 1");
        $st->execute([$order_id]);
        $row = $st->fetch();
        $paid_ok = $row && $row['status'] === 'paid';
    } catch (PDOException $e) {}
}
$pay_pending = $order_id !== '' && !$paid_ok;
?>
<!DOCTYPE html>
<html lang="ru">
<head>
...
<title>Оплата успешна — <?php echo $site_name; ?></title>
<style>
...
.card::after{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,rgba(16,185,129,0.5),transparent)}
.icon{width:74px;height:74px;margin:0 auto 22px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:30px;color:#6ee7b7;background:rgba(16,185,129,0.12);border:1px solid rgba(16,185,129,0.35);box-shadow:0 0 34px rgba(16,185,129,0.25)}
...
.order{font-size:12px;color:var(--faint);margin-bottom:26px}
...
<div class="card">
    ...
    <p>Спасибо за покупку! Подписка активирована автоматически — доступ уже выдан на ваш аккаунт.</p>
    <?php if ($pay_pending): ?>
        <p style="color:var(--dim);">Статус платежа ещё подтверждается — доступ активируется в течение минуты.</p>
    <?php endif; ?>
    <?php if ($order_id !== ''): ?>
        <div class="order">Заказ: <?php echo htmlspecialchars($order_id); ?></div>
    <?php endif; ?>
    <div class="btns">
        <a href="/profile" id="profileBtn" class="btn disabled"><i class="fas fa-user"></i> Перейти в профиль</a>
        <a href="https://t.me/kodexnull" id="contactBtn" class="btn-ghost" target="_blank" rel="noopener noreferrer"><i class="fas fa-paper-plane"></i> Связаться с нами</a>
    </div>
</div>
<script>
document.getElementById('contactBtn').addEventListener('click',function(){document.getElementById('profileBtn').classList.remove('disabled')});
</script>
